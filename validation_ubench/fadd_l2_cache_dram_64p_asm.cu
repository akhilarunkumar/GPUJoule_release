...
#define GLOBAL_MEM_ELEMENTS 195840
#define DRAM_GLO_MEM_ELEMENTS 19660800
...
__global__ void init_memory_2 (unsigned long long ** my_ptr_array_2, unsigned long long * my_array_2, int stride, int num_blocks_k, int num_threads_per_block_k) {
    ...
    void **ptr_array = (void **)my_ptr_array_2;
    unsigned long long *array = (unsigned long long *)my_array_2;

    if (tid == 0) {
        int elements_per_block = DRAM_GLO_MEM_ELEMENTS / num_blocks_k;
        int num_warps_per_block = num_threads_per_block_k / 32;
        int elements_per_warp = elements_per_block / num_warps_per_block;
        for (block_id = 0; block_id < num_blocks_k; block_id++) {
            for (warp_id = 0; warp_id < num_warps_per_block; warp_id++) {
                for (i = 0; i < elements_per_warp; i++) {
                    index = (block_id * elements_per_block) + (warp_id * elements_per_warp);
                    ptr_array[index + i] = (void*)&array[(index + ((i + 32) % elements_per_warp))];
                }
            }
        }
/*        for (i = 0; i < GLOBAL_MEM_ELEMENTS; i++) {
...
*/
        for (i = 0; i < DRAM_GLO_MEM_ELEMENTS; i++) {
            //array[i] = (unsigned long long)ptr_array[(i+stride)%GLOBAL_MEM_ELEMENTS];
            array[i] = (unsigned long long)ptr_array[i];
        }
    }
...
__global__ void shared_latency (unsigned long long ** my_ptr_array, unsigned long long ** my_ptr_array_2, unsigned long long * my_array, unsigned long long * my_array_2, int array_length, int iterations, unsigned long long * duration, int stride, int divergence, int num_blocks_k, int num_threads_per_block_k) {
...
//    int elements_per_warp = elements_per_block / num_warps_per_block;
    ...
    void **ptr_array = (void **)my_ptr_array;
    void **ptr_array_2 = (void **)my_ptr_array_2;
    unsigned long long int *array = (unsigned long long int *)my_array;
    unsigned long long int *array_2 = (unsigned long long int *)my_array_2;
    ...
    int elements_per_block = DRAM_GLO_MEM_ELEMENTS / num_blocks_k;
    num_warps_per_block = num_threads_per_block_k / 32;
    elements_per_warp = elements_per_block / num_warps_per_block;

    index1 = (block_id * elements_per_block) + (warp_id * elements_per_warp) + warp_thread_id;

    tmp_ptr_2 = (void **)(&(array_2[index1]));
...
*/           
            ...
            tmp_ptr_2 = (void**)(*tmp_ptr_2);
            f1 = f1 + (unsigned long long)(*tmp_ptr_2);
            tmp_ptr = (void**)(*tmp_ptr);
            f1 = f1 + (unsigned long long)(*tmp_ptr);
            tmp_ptr_2 = (void**)(*tmp_ptr_2);
            f1 = f1 + (unsigned long long)(*tmp_ptr_2);
...
void parametric_measure_shared(int N, int iterations, int stride) {
    ...
    unsigned long long ** h_ptr_b;
    unsigned long long ** d_ptr_b;
    ...
    h_b = (unsigned long long *)malloc(sizeof(unsigned long long int) * DRAM_GLO_MEM_ELEMENTS);

    h_ptr_a = (unsigned long long **)malloc(sizeof(unsigned long long int*)*N);

    h_ptr_b = (unsigned long long **)malloc(sizeof(unsigned long long int*)* DRAM_GLO_MEM_ELEMENTS);
    ...
    cudaMalloc ((void **) &d_b, sizeof(unsigned long long int) * DRAM_GLO_MEM_ELEMENTS );
    cudaMalloc ((void **) &d_ptr_a, sizeof(unsigned long long int*) * N );
    cudaMalloc ((void **) &d_ptr_b, sizeof(unsigned long long int*) * DRAM_GLO_MEM_ELEMENTS );
    ...
    cudaMemcpy((void *)d_b, (void *)h_b, sizeof(unsigned long long int) * DRAM_GLO_MEM_ELEMENTS, cudaMemcpyHostToDevice);
    cudaMemcpy((void *)d_ptr_a, (void *)h_ptr_a, sizeof(unsigned long long int *) * N, cudaMemcpyHostToDevice);
    cudaMemcpy((void *)d_ptr_b, (void *)h_ptr_b, sizeof(unsigned long long int *) * DRAM_GLO_MEM_ELEMENTS, cudaMemcpyHostToDevice);
    ...
    init_memory_2 <<<1, 1>>>(d_ptr_b, d_b, stride, num_blocks, num_threads_per_block);
    cudaDeviceSynchronize();
    ...
    std::string cmd = "GPUJOULE_DIR/nvml/example/power_monitor 5 > GPUJOULE_DIR/energy_model_ubench/energy_model_data/combined_inst_validation_data/fadd_l2_cache_dram_64p_asm_power.txt &";
    ...
    shared_latency <<<num_blocks, num_threads_per_block>>>(d_ptr_a, d_ptr_b, d_a, d_b, N, num_iterations, duration, stride, divergence, num_blocks, num_threads_per_block);
